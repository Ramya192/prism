# domains/financial_services/tools/drift_detector.py
# Isolation Forest -- unsupervised companion to ScamScorer (Tier 1) and
# WashTradingScorer (Tier 2), same purpose as
# domains/banking/fraud/tools/drift_detector.py: answers "does this
# record look statistically unlike training data," not "is this fraud."
# See that file's module docstring for the full rationale.
#
# One DriftDetector per tier (each tier is a different schema, so each
# needs its own forest over its own scorer's FEATURE_COLS) -- construct
...
TIER_DEFAULTS = {
    ...
    "tier2": (WashTradingScorer, "domains/financial_services/data/tier2_train.csv"),
}


class DriftDetector:
    def __init__(self, tier="tier1", data_path=None, contamination="auto"):
        self.scorer_cls, default_path = TIER_DEFAULTS[tier]
        self.tier = tier
    ...
    def _train(self, data_path, contamination):
        try:
            ...
            X = df[available].copy()
            for col in self.scorer_cls.CATEGORICAL_COLS:
                if col in X.columns:
    ...
    def _encode(self, record: dict) -> dict:
        ...
        for col in self.feature_cols:
            ...
            if col in self.scorer_cls.CATEGORICAL_COLS:
                encoder = self.encoders[col]
                val = "missing" if pd.isna(val) else str(val)  # same as training/score_batch
    ...
    def is_outlier(self, record: dict) -> bool:
        if not self.trained:
            return False
        X = pd.DataFrame([self._encode(record)])[self.feature_cols].fillna(0)
        return bool(self.model.predict(self.scaler.transform(X))[0] == -1)

    def anomaly_score(self, record: dict) -> float:
        if not self.trained:
            return 0.0
        X = pd.DataFrame([self._encode(record)])[self.feature_cols].fillna(0)
        return float(self.model.decision_function(self.scaler.transform(X))[0])
