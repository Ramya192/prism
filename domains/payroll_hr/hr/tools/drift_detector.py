# domains/payroll_hr/hr/tools/drift_detector.py
# Isolation Forest -- unsupervised companion to PostingFraudScorer
# (Tier 1), same purpose as domains/banking/fraud/tools/drift_detector.py:
# answers "does this posting look statistically unlike training data,"
...
class DriftDetector:
    def __init__(self, data_path="domains/payroll_hr/hr/data/train.csv", contamination="auto"):
        self.scaler = StandardScaler()
        self.encoders = {c: LabelEncoder() for c in PostingFraudScorer.CATEGORICAL_COLS}
        self.model = None
        self.trained = False
    ...
    def _train(self, data_path, contamination):
        try:
            ...
            available = [c for c in PostingFraudScorer.FEATURE_COLS if c in df.columns]
            if not available:
                print("  [DriftDetector] No usable feature columns found. Disabled.")
                return

            X = df[available].copy()
            for col in PostingFraudScorer.CATEGORICAL_COLS:
                if col in X.columns:
    ...
    def _encode(self, record: dict) -> dict:
        record = PostingFraudScorer._derive_features(record)
        row = {}
        for col in self.feature_cols:
            val = record.get(col, 0)
            if col in PostingFraudScorer.CATEGORICAL_COLS:
