"""
domains/banking/fraud/eval_harness.py — ground-truth evaluation of the DEPLOYED
...
domain's own data/ folder, and this is what configs/banking.yaml's
...
Usage (from the prism/ repo root):
    python domains/banking/fraud/eval_harness.py                  # deterministic layers only (free)
    python domains/banking/fraud/eval_harness.py --with-llm-sample  # + a small real-LLM sample (costs API calls)

Writes EVAL_RESULTS.md in this domain's folder either way. Requires
domains/banking/fraud/data/train.csv and test_holdout.csv — regenerate with
prepare_data.py first (not committed; see that script's docstring).

Also runs Tier 2 (Transaction_Amount/Account_Age/Credit_Score schema) via
the shared engine in core/eval/base_fraud_eval_harness.py — see
run_tier2_eval() below — appending its own section to the same
EVAL_RESULTS.md. Requires domains/banking/fraud/data/tier2_train.csv and
tier2_test_holdout.csv (regenerate with data/prepare_tier2_data.py).

Run as a module so `core`/`domains` resolve, e.g. from the prism/ repo root:
    python -m domains.banking.fraud.eval_harness
    python -m domains.banking.fraud.eval_harness --with-llm-sample
"""
...
from core.eval.base_fraud_eval_harness import TierSpec, render_markdown as render_tier2_markdown, run_tier_eval
...
def run_tier2_eval() -> dict:
    """Tier 2 (GeneralizableScorer's Transaction_Amount/Account_Age/
    Credit_Score schema) — run_deterministic_eval() above only covers
    Tier 1, so this covers Tier 2 via the shared engine
    (core/eval/base_fraud_eval_harness.py), the same one every other
    domain's own Tier 1/2 split uses. `_normalize_for_rules` is
    detector_agent.py's own Tier-2-to-canonical-field remap
    (Transaction_Amount -> Amount) — reused as-is, not re-derived, so the
    rule layer being evaluated here is exactly the one Tier 2
    transactions actually hit."""
    def tier2_rule_fn(transaction: dict):
        normalized = FraudDetectorAgent._normalize_for_rules(transaction)
        if normalized is None:
            return None
        return FraudDetectorAgent.rule_based_filter(normalized)

    tier2 = TierSpec(
        name="Tier 2 — Transaction_Amount/Account_Age/Credit_Score schema",
        holdout_path=str(_HERE / "data" / "tier2_test_holdout.csv"),
        label_col="Is_Fraud",
        rule_fn=tier2_rule_fn,
        scorer=GeneralizableScorer(),
    )
    return run_tier_eval(tier2)
...
def run_llm_sample_eval(seed: int = 42) -> dict | None:
    ...
    agent = FraudDetectorAgent(name="EvalHarnessAgent", data_path=str(TRAIN_PATH))
    y_true, y_pred = [], []
...
def render_markdown(deterministic: dict, llm_sample: dict | None) -> str:
    d = deterministic
    lines = [
        "# Evaluation Results — Fraud Detection (banking domain)",
        "",
    ...
    lines += [
        "---",
        "*Regenerate: `python domains/banking/fraud/eval_harness.py` (add `--with-llm-sample` for the LLM-sample "
...
def main():
    ...
    print("\nEvaluating Tier 2 — Transaction_Amount/Account_Age/Credit_Score schema...")
    tier2 = run_tier2_eval()
    print(f"  Combined precision={tier2['combined_deterministic']['precision']:.4f} "
          f"recall={tier2['combined_deterministic']['recall']:.4f} f1={tier2['combined_deterministic']['f1']:.4f}")

    markdown = render_markdown(deterministic, llm_sample)
    markdown += "\n" + render_tier2_markdown(
        "Banking (fraud detection) — Tier 2",
        "Tier 1's own numbers are above (this domain's original eval_harness.py, unchanged). "
        "Tier 2 uses the shared engine in core/eval/base_fraud_eval_harness.py, same as every "
        "other domain's Tier 1/2 split.",
        [tier2],
    )
