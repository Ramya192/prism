...
1. Top-level batch upload within *different* domains at once is now built (above); a genuinely bigger scope still deferred by choice: there is no cross-*batch* dedup/merge — uploading the same file twice in two separate batches ingests it twice, same as the existing single-file flow already does.
